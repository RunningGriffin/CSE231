...
public:
    ...
    Piece(const Position& pos, bool isWhite = true)
        : fWhite(isWhite), nMoves(0), lastMove(0), position(pos) {
    }
    Piece(int c, int r, bool isWhite = true)
        : fWhite(isWhite), nMoves(0), lastMove(0), position(c, r) {
    }
    Piece(const Piece& piece)
        : fWhite(piece.fWhite), nMoves(piece.nMoves),
        lastMove(piece.lastMove), position(piece.position) {
    }
    virtual ~Piece() {}
